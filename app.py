...
app = Flask(__name__) 
API_KEY = 'ENTER-YOUR-API-KEY' #THIS IS THE ACTUAL API KEY DONT GET CONFUSED.
...
def home(): 
    ...
    c = con.cursor()
    rows = c.execute('SELECT country_code, country_name FROM country;').fetchall()
    ...
    if request.method == 'POST': 
        try: 
            ...
            url = 'https://www.alphavantage.co/query?function=CURRENCY_EXCHANGE_RATE&from_currency={}&to_currency={}&apikey={}'.format( 
                from_c, to_c, API_KEY)
            #https://www.alphavantage.co/query?function=CURRENCY_EXCHANGE_RATE&from_currency=ALL&to_currency=BHD&apikey=ENTER-YOUR-API-KEY
            ...
            return render_template('home.html', rows=rows, result=round(result, 2), amount=amount, 
